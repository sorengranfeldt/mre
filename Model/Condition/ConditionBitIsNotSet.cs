namespace Granfeldt
{
    ...
    public class ConditionBitIsNotSet : ConditionBase
    {
        ...
        public override bool Met(MVEntry mventry, CSEntry csentry)
        {
            ...
            if (set)
            {
                Tracer.TraceInformation("Condition failed (Reason: Bit {0} in attribute {1} is set) {2} ", this.BitPosition, this.MVAttribute, this.Description);
                return false;
            }
            return true;
        }
